...
def run_weight_perturb(work_dir, gth_pp_file, cp2k_exe, parallel_exe, element, \
  ...
  if ( max_cycle != 2000 ):
    process_3_dir = ''.join((work_dir, '/process_mix'))
  else:
    process_3_dir = ''.join((work_dir, '/process_3'))
  if ( (not os.path.exists(process_3_dir)) and max_cycle != 2000 ):
...
weight_pertburb_new=`eval echo '$'"weight_perturb_choice_$choice"`
$direc/optimize.sh "${weight_pertburb_new}" "${converge_standard}" $j old $m
...
((n=$n-2))
weight_pertburb_new=`eval echo '$'"weight_perturb_choice_$choice"`
$direc/optimize.sh "${weight_pertburb_new}" "${converge_standard}" $j new $n
((k=base_1+(($i-base_2))*2+2))
...
((j=base_1+(($i-base_2))*2+1))
weight_pertburb_new=`eval echo '$'"weight_perturb_choice_$choice"`
$direc/optimize.sh "${weight_pertburb_new}" "${converge_standard}" $j new $m
((k=base_1+(($i-base_2))*2+2))
value_2=`$direc/optimize.sh "${weight_standard}" "${converge_standard}" $k new $j`
((m=$m+2))
n=$m
...
''' %(process_3_dir, weight_1[0], weight_1[1], weight_1[2], target_semi, target_val, target_vir, \
      weight_pertub_1_str, weight_pertub_2_str, weight_pertub_3_str, weight_pertub_4_str, max_cycle)
...
''' %(process_3_dir, parallel_exe, element, method, val_elec_num, python_exe, get_min_index, \
  ...
  perturb_file = ''.join((process_3_dir, '/perturb.sh'))
  ...
  while p.poll() is None:
    ...
    if ( 4 in choice ):
      ...
      if ( index_3[len(index_3)-1] > index_4[len(index_4)-1] ):
...
if __name__ == '__main__':
  ...
  gth_pp_file = '/home/lujunbo/code/github/CP2K_kit/example/gth_pp_opt/process_2/restart6/step_2/GTH-PARAMETER'
  ...
  weight_perturb.run_weight_perturb(work_dir, gth_pp_file, cp2k_exe, parallel_exe, \
