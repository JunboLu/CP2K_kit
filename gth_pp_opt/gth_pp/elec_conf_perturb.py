...
def run_elec_config_perturb(work_dir, gth_pp_file, cp2k_exe, parallel_exe, element, \
                            method, val_elec_num, python_exe, get_min_index, consider_charge, choice_num, elec_config, \
                            elec_config_perturb_choice_1, elec_config_perturb_choice_2, \
                            elec_config_perturb_choice_3, elec_config_perturb_choice_4, \
  ...
  #make process_5 directory, and then copy initial guess gth pp file in process_5. 
  #The initial guess gth pp file for process_5 is produced by process_4.
  if ( max_cycle != 2000 ):
    process_5_dir = ''.join((work_dir, '/process_mix'))
  else:
    process_5_dir = ''.join((work_dir, '/process_5'))
  if ( (not os.path.exists(process_5_dir)) and max_cycle != 2000 ):
    subprocess.run('mkdir process_mix', cwd=work_dir, shell=True)
  if ( (not os.path.exists(process_5_dir)) and max_cycle == 2000 ):
    subprocess.run('mkdir process_5', cwd=work_dir, shell=True)
  ...
  if ( restart_num != 0 ):
    cmd = "ls | grep %s" %('bak_')
    bak_num = len(call.call_returns_shell(process_5_dir, cmd))
    bak_dir = ''.join((process_5_dir, '/bak_', str(bak_num+1)))
...
elec_perturb_choice_1='%s'
elec_perturb_choice_2='%s'
elec_perturb_choice_3='%s'
elec_perturb_choice_4='%s'
choice_num=%d
...
$direc/optimize.sh "${elec_perturb_choice_1}" "${converge_standard}" 1
value_1=`$direc/optimize.sh "${elec_standard}" "${converge_standard}" 2`
echo 2 $value_1 $choice
echo 2 $value_1 >> $direc/$filename

$direc/optimize.sh "${elec_perturb_choice_1}" "${converge_standard}" 3 new 2
value_2=`$direc/optimize.sh "${elec_standard}" "${converge_standard}" 4 new 3`
...
elec_pertburb_new=`eval echo '$'"elec_perturb_choice_$choice"`
$direc/optimize.sh "${elec_pertburb_new}" "${converge_standard}" $j old $m
((k=base_1+(($i-base_2))*2+2))
value_2=`$direc/optimize.sh "${elec_standard}" "${converge_standard}" $k old $j`
((m=$m+2))
n=$m
...
if [ $choice -lt choice_num ]; then
((choice=$choice+1))
elif [ $choice == $choice_num ]; then
choice=1
fi
...
((n=$n-2))
elec_pertburb_new=`eval echo '$'"elec_perturb_choice_$choice"`
$direc/optimize.sh "${elec_pertburb_new}" "${converge_standard}" $j new $n
((k=base_1+(($i-base_2))*2+2))
value_2=`$direc/optimize.sh "${elec_standard}" "${converge_standard}" $k new $j`
((n=$n+2))
((m=$m+2))
...
((j=base_1+(($i-base_2))*2+1))
elec_pertburb_new=`eval echo '$'"elec_perturb_choice_$choice"`
$direc/optimize.sh "${elec_pertburb_new}" "${converge_standard}" $j new $m
((k=base_1+(($i-base_2))*2+2))
value_2=`$direc/optimize.sh "${elec_standard}" "${converge_standard}" $k new $j`
((m=$m+2))
n=$m
...
sed -ie "${line_elec_config}s/.*/  ELECTRON_CONFIGURATION $elec_str/" $direc/restart$i/step*/atom.inp
...
echo $value
cd $direc
''' %(process_5_dir, parallel_exe, element, method, val_elec_num, python_exe, get_min_index, consider_charge, \
      line_step_size[0], line_elec_config[0], line_tg_semi[0], line_tg_val[0], line_tg_vir[0], line_pot_file[0], cp2k_exe)
  ...
  perturb_file = ''.join((process_5_dir, '/perturb.sh'))
  with open(perturb_file, 'w') as f:
    f.write(perturb)

  subprocess.run('chmod +x perturb.sh', cwd=process_5_dir, shell=True)
  subprocess.run('chmod +x optimize.sh', cwd=process_5_dir, shell=True)
  out_temp=tempfile.TemporaryFile(mode='w+')
  fileno=out_temp.fileno()
  p = subprocess.Popen("bash -c './perturb.sh'", cwd=process_5_dir, shell=True, stdout=fileno, stderr=fileno)
