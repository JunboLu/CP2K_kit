...
def calc_v_hartree(cube_file, surface, work_dir):
  ...
  tips = '''
Tips:
(1) v_hartree in CP2K = the electrostatic potential generated by nuclei (or ions) +
    the electrostatic potential generated by electrons. The latter is usually refereed
    as hartree potential while the former is refered as an external potential in the
    literature. In principle, one needs to use v_hartree + v_xc. However, v_xc decays
    much faster than v_hartree so that if electron goes a bit far away from a surface,
    there is only only v_hartree.
(2) CP2K does not give you the correct fermi level for a semiconductor as by default it
    considers the HOMO as the fermi level. So you have to be careful if you are working
...
def v_hartree_run(hartree_param, work_dir):
  ...
  if ( 'surface' in hartree_param.keys() ):
    ...
    surface = [int(x) for x in surface]

  calc_v_hartree(cube_file, surface, work_dir)
