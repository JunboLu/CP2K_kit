...
def load_data_from_dir(proj_dir, work_dir, save_dir, proj_name):
  ...
  if os.path.exists(md_cell_file):
    for i in range(frames_num):
      line_i = linecache.getline(md_cell_file, i+2)
      line_i_split = list_dic_op.str_split(line_i, ' ')
    ...
    if os.path.exists(restart_file):
      cmd = "grep -n %s %s" %("'&CELL'", restart_file)
  ...
  energy_file = open(''.join((save_dir, '/energy.raw')), 'w')
  for i in range(frames_num):
    line_i = linecache.getline(md_pos_file, i*(atoms_num+2) + 2)
    line_i_split = list_dic_op.str_split(line_i, ' ')
  ...
  for i in range(frames_num):
    frame_str = ''
    for j in range(atoms_num):
      line_ij = linecache.getline(md_pos_file, i*(atoms_num+2)+2+j+1)
      line_ij_split = list_dic_op.str_split(line_ij, ' ')
      if (j==0):
        frame_str = ' '.join((line_ij_split[1], line_ij_split[2], line_ij_split[3].strip('\n')))
      else:
  ...
  if os.path.exists(md_pos_file):
    for i in range(frames_num):
      frame_str = ''
      for j in range(atoms_num):
        line_ij = linecache.getline(md_frc_file, i*(atoms_num+2)+2+j+1)
        line_ij_split = list_dic_op.str_split(line_ij, ' ')
  ...
  if os.path.exists(stress_file):
    ...
    for i in range(frames_num):
      ...
      line_i = linecache.getline(stress_file, i+2)
      line_i_split = list_dic_op.str_split(line_i, ' ')
...
def raw_to_set(data_dir, parts):

  '''
  raw_to_set : divide the raw data into several parts.
